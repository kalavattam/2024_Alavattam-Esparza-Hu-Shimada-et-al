...
#  Fig-S6D_MDC1-on-XY_Setdb1_pachynema.R
#  KA
...
file_path <- file.path(
    dir_script, "tsvs", "Fig-S6D_MDC1-on-XY_Setdb1_pachynema.tsv"
)
data <- readr::read_tsv(file_path, show_col_types = FALSE) %>%
    #  Filter out rows where the total number of observations is zero
    dplyr::filter(S6D_observed_pachynema > 0) %>%
    #  Calculate the percentage
    dplyr::mutate(
        percentage =
            `S6D_MDC1-on-XY_Setdb1_pachynema` /
            S6D_observed_pachynema * 100,
...
t_test_result$p.value  # [1] 0.06537801
...
summary_data <- data %>%
    ...
    dplyr::summarize(
        ...
        n = sum(S6D_observed_pachynema), .groups = 'drop'
    ) %>%
    dplyr::mutate(
        sem = sd / sqrt(n)
    )
...
ggplot2::ggsave(
    file.path(
        dir_script, "plots", "Fig-S6D_MDC1-on-XY_Setdb1_pachynema_SD.pdf"
    ),
...
ggplot2::ggsave(
    file.path(
        dir_script, "plots", "Fig-S6D_MDC1-on-XY_Setdb1_pachynema_SEM.pdf"
    ),
