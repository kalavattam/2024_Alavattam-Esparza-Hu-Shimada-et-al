...
#  Fig-S6B_SETDB1-any-on-X-PCH_Mdc1_pachynema.R
#  KA
...
file_path <- file.path(
    dir_script, "tsvs", "Fig-S6B_SETDB1-any-on-X-PCH_Mdc1_pachynema.tsv"
)
data <- readr::read_tsv(file_path, show_col_types = FALSE) %>%
    #  Filter out rows where the total number of observations is zero
    dplyr::filter(S6B_observed_pachynema > 0) %>%
    #  Calculate the percentage
    dplyr::mutate(
        percentage =
            `S6B_SETDB1-any-on-X-PCH_Mdc1_pachynema` /
            S6B_observed_pachynema * 100,
        genotype = factor(genotype, levels = c("Ctrl", "KO"))
    )
...
t_test_result$p.value  # [1] 0.001132189
...
ggplot2::ggsave(
    file.path(
        dir_script, "plots",
        "Fig-S6B_SETDB1-any-on-X-PCH_Mdc1_pachynema_SD.pdf"
    ),
...
ggplot2::ggsave(
    file.path(
        dir_script, "plots",
        "Fig-S6B_SETDB1-any-on-X-PCH_Mdc1_pachynema_SEM.pdf"
    ),
