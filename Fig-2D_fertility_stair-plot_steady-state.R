...
#  Fig-2E_fertility_stair-plot_steady-state.R
#  KA
#  2020-0812, 2024-0210
...
tsv <- "Atf7ip2_fertility_steady-state.tsv"
tbl <- readr::read_tsv(
...
#  Set image dimensions and other information, then save the plot
image_w <- 6
image_h <- 4
ggplot2::ggsave(
    filename = file.path(
        dir_script, "plots", "Fig-2E_fertility_stair-plot_steady-state.pdf"
    ),
    plot = q,
    units = "in",
    width = image_w,
