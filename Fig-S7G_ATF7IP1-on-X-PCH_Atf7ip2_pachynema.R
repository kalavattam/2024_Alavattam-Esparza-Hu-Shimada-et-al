...
#  Fig-S7G_ATF7IP1-on-X-PCH_Atf7ip2_pachynema.R
#  KA
...
file_path <- file.path(
    dir_script, "tsvs", "Fig-S7G_ATF7IP1-on-X-PCH_Atf7ip2_pachynema.tsv"
)
data <- readr::read_tsv(file_path, show_col_types = FALSE) %>%
    #  Filter out rows where the total number of observations is zero
    dplyr::filter(S7G_observed_pachynema > 0) %>%
    #  Calculate the percentage
    dplyr::mutate(
        percentage =
            `S7G_ATF7IP1-on-X-PCH_Atf7ip2_pachynema` /
            S7G_observed_pachynema * 100,
...
t_test_result$p.value  # [1] 0.007119247
...
summary_data <- data %>%
    ...
    dplyr::summarize(
        ...
        n = sum(S7G_observed_pachynema), .groups = 'drop'
    ) %>%
    dplyr::mutate(
        sem = sd / sqrt(n)
    )
...
ggplot2::ggsave(
    file.path(
        dir_script, "plots", "Fig-S7G_ATF7IP1-on-X-PCH_Atf7ip2_pachynema_SD.pdf"
    ),
...
ggplot2::ggsave(
    file.path(
        dir_script, "plots", "Fig-S7G_ATF7IP1-on-X-PCH_Atf7ip2_pachynema_SEM.pdf"
    ),
